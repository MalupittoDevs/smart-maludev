...
export default function Ventas() {
  ...
  const addToCart = () => {
    ...
    setCart((prev) => {
      ...
      if (i >= 0) {
        ...
        if (newQty > prod.qty) {
          push({ type: "error", msg: `No puedes superar el stock (${prod.qty})` });
          return prev;
        }
    ...
    setCart((prev) =>
      prev.map((l) =>
        l.product.id === id
          ? {
              ...l,
              qty: Math.max(1, Math.min(qty, l.product.qty)), // entre 1 y stock
            }
          : l
      )
    );
  ...
  const confirmPurchase = async () => {
    if (cart.length === 0) return push({ type: "error", msg: "Carrito vacío" });
    ...
    <div style={{ display: "grid", gridTemplateColumns: "1fr 320px", gap: 16 }}>
      ...
      <div>
        ...
        >
          ...
          />
          <button onClick={addToCart} disabled={loading} style={btnPrimary}>
            ➕ Agregar
          </button>
          <button onClick={() => void loadProducts()} disabled={loading} style={btnGhost}>
            🔄 Catálogo
          </button>
        </div>
        ...
        <div style={{ overflowX: "auto" }}>
          <table style={table}>
            ...
            <tbody>
              {cart.length === 0 ? (
                <tr>
                  <td colSpan={6} style={{ padding: 16, opacity: 0.6, textAlign: "center" }}>
                    Agrega productos por SKU para comenzar
                  </td>
                </tr>
              ) : (
                cart.map((l) => (
                  <tr key={l.product.id} style={{ borderTop: "1px solid #131b25" }}>
                    <td style={td}>{l.product.name}</td>
                    <td style={td}>{l.product.sku}</td>
                    <td style={{ ...td, textAlign: "right" }}>
                      <input
                        ...
                        max={l.product.qty}
                        value={l.qty}
                        onChange={(e) => updateLineQty(l.product.id, Number(e.target.value))}
                        style={{ ...input, width: 100, textAlign: "right" }}
                      />
                    </td>
      ...
      >
        <h2 style={{ margin: "4px 0 8px 0" }}>TOTAL FINAL</h2>
        ...
        <div style={{ ...summaryRow, borderTop: "1px solid #203048", paddingTop: 10 }}>
          <span>Total</span>
          <b>{formatCLP(total)}</b>
        </div>
        ...
        <button onClick={clearCart} disabled={cart.length === 0 || confirming} style={btnGhost}>
          Limpiar carrito
        </button>
        ...
        <div
          style={{
            ...
            fontSize: 12,
            opacity: 0.8,
            borderTop: "1px dashed #203048",
            paddingTop: 8,
          }}
        >
          <b>Panel</b> para métricas, insights o predicciones (placeholder).
        </div>
      </aside>
    </div>
  );
